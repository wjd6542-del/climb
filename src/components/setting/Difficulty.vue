...
const {
  ...
  saveRows,
  deleteRows,
} = useAgGridCrud<Difficulty>({
  loader: () => difficultyService.list<Difficulty[]>(),
  saver: (rows) => difficultyService.batchSave(rows),
  deleter: (rows) => difficultyService.batchDelete(rows),
  newRowFactory: (id) => ({
    id,
    name: "",
    code: "",
